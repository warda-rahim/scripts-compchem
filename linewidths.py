...
parser = argparse.ArgumentParser(
         description='Plots phonon linewidths vs frequency at a specific temperature')
...
#linewidths = [(2 * 2 * np.pi * i) for i in gamma_1D]

#print("min-linewidth:", np.min(linewidths))
#print("max-linewidth:", np.max(linewidths))



ax.tick_params(axis='both', which='both', direction='in', length=24, width=2, labelsize=50, pad=15)
...
ax.set_ylabel(r'$\mathregular{\Gamma_{\lambda} (THz)}$', fontsize=50)



def generate_cmap(cmax='#E75480', cmin='#A6D608', alpha=1, density=512):
...
plt.scatter(freq_1D, gamma_1D, s=5, facecolor=newcolors) 

plt.subplots_adjust(left = 0.19, right = 0.98, top  = 0.95, bottom = 0.15)
plt.savefig('linewidths_vs_freq-{}K-{}.pdf'.format(args.temp, args.output))
plt.savefig('linewidths_vs_freq-{}K-{}.png'.format(args.temp, args.output), dpi=500)
plt.show()
