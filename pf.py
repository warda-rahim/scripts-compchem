...
plt.plot(pf[1:,0], pf[1:,1:], linewidth=1.5, markersize=8)

plt.legend(pf[0,1:], loc='upper center', ncol=5, bbox_to_anchor=(0.5, 1.15), frameon=False, title='Carrier Concentration ($\mathregular{cm^{-3}}$)') 

plt.xlabel('Temperature (K)', fontsize=14)
plt.ylabel('PF ($\mathregular{\mu}$W $\mathregular{cm^{-1}K^{-2}}$)', fontsize=14)

plt.subplots_adjust(left=0.15,right=0.9,bottom=0.1,top=0.9)

plt.savefig('pf-n-type.pdf')
plt.savefig('pf-n-type.png', dpi=500)
plt.show()
